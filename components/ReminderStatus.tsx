...
import { Bell, CalendarClock, MailCheck, ShieldAlert, Sparkles, Copy, Server, Activity, Send, Loader2, CheckCircle2 } from 'lucide-react';
...
Equipe Técnica`;
  ...
  const handleManualSend = () => {
    if (sentSuccess) return; // Prevent double sending
    ...
    setTimeout(() => {
      ...
      alert(`E-mail enviado com sucesso para ${emails.length} destinatários.`);
    }, 2000);
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-slate-200">
              {sentSuccess ? 'E-mail Enviado com Sucesso' : 'Notificação Pendente'}
            </p>
            <p className="text-xs text-slate-500">
              {sentSuccess ? `Enviado às ${lastSentTime}` : 'Aguardando gatilho automático ou manual'}
            </p>
          </div>
        </div>
