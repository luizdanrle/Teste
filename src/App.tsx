...
const QrModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  const currentUrl = "https://ai.studio/apps/drive/1eI-muCQOZsa0GDMhNmoPzZftWbg5ZAez?fullscreenApplet=true";
...
const App: React.FC = () => {
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      const canvas = await html2canvas(element, {
        scale: 3, // Ultra Resolution
        useCORS: true, 
        allowTaint: true,
        logging: false,
        backgroundColor: '#f8fafc', // match bg-slate-50
        windowWidth: 1200, // Force desktop width layout
        // Remove elementos que não devem sair na impressão (se houver classe no-print)
        ignoreElements: (el) => el.classList.contains('no-print')
      });

      const link = document.createElement('a');
      link.download = `Relatorio-Tecnico-${serviceInfo.id}.png`;
      link.href = canvas.toDataURL('image/png'); // Lossless PNG
      link.click();
    } catch (err) {
      console.error("Erro ao gerar imagem:", err);
      alert("Aviso: Algumas imagens externas podem estar bloqueadas pelo navegador. O relatório será baixado, mas algumas fotos podem não aparecer.");
    } finally {
      setIsGenerating(false);
    }
  };

  // Tratamento de erro de imagem Robusto
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
    const img = e.currentTarget;
    // Proteção contra loop infinito
    if (img.dataset.retry === 'true') {
      // Se falhar de novo, coloca um placeholder
      img.src = "https://placehold.co/400x400?text=Imagem+Indisponivel";
      return;
    }

    // Se a URL for do proxy, tenta carregar direto do Google Drive
    if (img.src.includes('wsrv.nl')) {
      const originalUrl = img.src.split('?url=')[1];
      if (originalUrl) {
         img.dataset.retry = 'true'; // Marca que já tentou recuperar uma vez
         img.src = decodeURIComponent(originalUrl);
         // Remove crossorigin se falhar o proxy para evitar bloqueio total
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20 font-sans">
      ...
      {/* Header */}
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm backdrop-blur-md bg-opacity-90 no-print">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <Section title="Técnico Responsável" icon={<User size={20} />}>
            <div className="flex items-start gap-5">
              <img 
                src={getCorsProxyUrl(provider.avatarUrl)} 
                alt="Tech" 
        ...
        <Section title="Registro Fotográfico" icon={<ImageIcon size={20} />}>
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-2">
             {GALLERY_IMAGES.map((src, index) => (
               <div 
                  key={index} 
                  className="group relative aspect-[4/3] bg-slate-100 rounded-xl overflow-hidden shadow-sm cursor-pointer border border-slate-200 hover:border-tech-400 transition-all hover:shadow-lg hover:-translate-y-1"
                  onClick={() => openLightbox(index)}
               >
                 <img 
                   src={getCorsProxyUrl(src)} 
                   alt={`Serviço ${index + 1}`} 
                   crossOrigin="anonymous"
                   onError={handleImageError}
        ...
        <section className="mt-12 space-y-6">
           ...
           <div className="flex flex-col items-center justify-center mt-16 mb-6 no-print">
             <div className="relative px-8 py-4">
                <p className="font-serif text-2xl italic text-slate-800 text-center relative z-10">Luiz Danrley Santos Silva</p>
                <div className="absolute bottom-4 left-0 w-full h-px bg-slate-300"></div>
             </div>
             <p className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mt-1">Assinado Digitalmente - Responsável Técnico</p>
             <p className="text-[10px] text-emerald-600 font-medium mt-1 flex items-center gap-1">
               <CheckCircle2 size={10} /> Documento Válido para Garantia
             </p>
           </div>
